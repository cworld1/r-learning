...
# 如果想要数据的话也可以用 dplyr::count() 统计
# （注意所有 count() 能做的，group_by + summarise 都能）
diamonds %>%
...
# geom_histogram() 生成的是柱形图，但入宫你想要叠加多个数据的话，更推荐能生成线条图的 geom_freqpoly()：
...
# 这些值带着明显的伦理问题和常理性错误。而我们也最好是在没有异常值的情况下进行分析。
# 如果它们对结果的影响很小，并且我们尚且无法弄清楚它们存在的原因，那么用缺失的值（NA）替换它们是合理的。
# 但如果它们对我们的结果有实质性影响，则不应当无缘无故地放弃它们。
# 我们需要找出导致它们的原因（如数据输入错误），并合理地处理这些异常数据。

# 【实战】处理 diamonds 的异常数据 ----
...
# 事实上，如果数据含有 NA，ggplot 绘图会发出警告并将相应数据剔除不会展示出来
...
# 【实战】diamonds 数据集统计与绘图 ----

# 连续变量多类绘图 1：频率多边形----
# 正如前面的频率多边形那样，我们通常需要观察按不同类别的连续变量的分布直方图。
# 所以我们采用频率多边形来展示：
ggplot(data = diamonds, mapping = aes(x = price)) +
    geom_freqpoly(mapping = aes(colour = cut), binwidth = 500)
# 但不同类别之间的数据差异依然相对明显，一些相对 “小得多” 的类别的形状难以被观察到。
# 使用柱状图可以明显看到各个种类计数差异巨大：
ggplot(diamonds) +
    geom_bar(mapping = aes(x = cut))
# 因此我们需要修改 y 轴的内容。我们将不显示计数，而是显示密度（使其标准化），以便每个频率多边形围成的面积正好为 1
ggplot(data = diamonds, mapping = aes(x = price, y = ..density..)) +
    geom_freqpoly(mapping = aes(colour = cut), binwidth = 500)
# 实际结果令人震惊！品质最差钻石却有着最高的平均价格！（UC：明天来上班）这个图中有很多有趣的现象，我们将继续挖掘。
...
# 它支持了一个违反直觉的发现，即质量更好的钻石明显更便宜！

# 像 cut 这样的变量可能还有一个较好的默认排序，但事实上我们遇到的很多数据集可能没有这么好，需要重新排序
# reorder() 可以帮你做到。如 mpg 数据集的 class：
ggplot(data = mpg, mapping = aes(x = class, y = hwy)) +
    geom_boxplot()
# 为了使趋势更容易看到，我们可以根据的中值重新排序：hwy
ggplot(data = mpg) +
