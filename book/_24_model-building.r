...
# 上一章我们侧重于模拟数据集。本章将重点介绍真实数据，以及如何构建模型以便理解数据。
# 我们可以将数据划分为模式和残差模型。注意模型建立有时不尽人意，有时建立得很好但你不知道为什么。
# 如果你遇到了任何困惑 —— 请不要畏惧重新开始！换个全新思路可能对你非常有用。

# 【实战】分析 diamonds 品质与价格反常的原因 ----
# 还记得我们之前研究的奇怪现象吗？品质越差的钻石，价格越高。
# 不只是质量，在颜色、纯净度方面都是这样表现的：
...
mod_diamond <- lm(lprice ~ lcarat, data = diamonds_new)
grid <- diamonds_new %>%
    data_grid(carat = seq_range(carat, 20)) %>% # 创建矩阵
    mutate(lcarat = log2(carat)) %>% # 矩阵生成的 carat 也需要取对数
    add_predictions(mod_diamond, "lprice") %>% # 预测
...
# 这告诉我们一些关于这些数据的有趣信息：
# 如果我们相信我们创造的模型，那么大钻石比预期的要便宜得多。

# 我们来分析残差的数据：
diamonds_new <- diamonds_new %>%
    add_residuals(mod_diamond, "lresid") # 注意这里分析得到的数据是取过对数的
...
ggplot(diamonds_new, aes(lcarat, lresid_new)) +
    geom_hex(bins = 50)
# 我们会发现有一些钻石数据的残差相当大 —— 尤其是因为取对数的关系，
# 残差为 2 表示钻石的价格是我们预期的 4 倍。单独查看异常值通常很有用：
diamonds_new %>%
...
# 似乎没有什么出人意料的数据，但可能值得花时间考虑，
# 这是否表明我们的模型存在问题，或者数据中存在错误。

# 【实战】分析 flights 每日航班数量变化因素 ----
...
ggplot(daily, aes(wday, n)) + # 注意这里默认周末排最前
    geom_boxplot()
# 双休日的航班相对要少些，因为大多数旅行都是商务旅行。
# 这种影响在周六尤为明显：你有时可能会在周日离开去参加周一上午的会议，
# 但你很少在周六离开，因为你更愿意在家和家人们在一起。

# 消除这种影响最简单的方法是使用模型矫正。
# 首先，我们拟合模型，并将其预测覆盖在原始数据上来可视化：
...
daily %>%
    ...
    geom_line()
# 现在，通过星期预估模型，我们发现航班数量仍然明显偏离了预期航班数量。
# 由于现在我们已经消除了大部分大的每周效应，但我们可以看到一些更微妙的模式仍然存在：
# 有没有一种可能，我是说可能，这个模型的变化不只是单纯的被星期几？
# 从6月份开始，我们的模型很是失败。为一周中的每一天绘制一条线的情节，使原因更容易看到：
ggplot(daily, aes(date, resid, colour = wday)) +
    geom_ref_line(h = 0) +
    geom_line()
# 有些线条仍然在乱飞！尤其是周六的航班数量：夏季的航班比我们预期的要多，秋季的航班则少得多。
# 之后我们会着重分析。

# 将偏离过大的 “异常” 数据单独取出仔细研究：
daily %>%
    filter(resid < -100)
#> # A tibble: 11 x 4
#>   date           n wday  resid
...
#> # … with 5 more rows
# 请注意这些日期并不是巧合。从美国公共假日角度来看，
# 我们会发现这些数据包含元旦、美国独立日（7月4日）、感恩节和圣诞节。
...
# 冬季（12月至3月）的航班相对较少，而夏季（5月至9月）的航班明显更多。
# 尽管我们只有一年的数据，没有更好地佐证这个观点。但我们可以通过思考获取潜在的解释。
...
# 当地人们通常在夏天去度假，也不介意选择周六度假。
# 选择这个时间段度假，很可能是因为孩子们正好在放暑假。
# 所以我们有理由推测暑假是从6月初到8月下旬。
# 实际查询数据我们得到，该州在2013年的暑假安排是6月26日至9月9日。
# 但为什么春季的周六航班比秋季多？调研得知计划秋天度假的家庭实在不太常见，因为感恩节和圣诞节假期真的已经足够长。
...
daily %>%
    ...
    geom_boxplot()
# 每个星期几都呈现这样的趋势：春季数据多变（由于节日，春季开头与结尾差异较大）、夏季（数据）航班偏多。
# 周六用于暑假度假的趋势也很明显。
...
# 针对一年中的不同时间点 ----
# 我们可以使用一个更为灵活的模型，用来捕获我们感兴趣的内容。
# 一个简单的线性趋势是不够的，我们可以尝试使用自然样条来拟合全年的平滑曲线：
...
# 这两种方法都是合理的。但如果要画图或检查变量，显式转换可能更好。
# 但请不要随意使用返回多个列的转换（如样条曲线）。
...
#       by Gareth James, Daniela Witten, Trevor Hastie
#   http://www-bcf.usc.edu/~gareth/ISL/（可以免费在线阅读，甚至还有中文版本）。
#   本书介绍了一系列现代建模技术，统称为统计学习。
#   要更深入地了解模型背后的数学原理，则请阅读：
#   Elements of Statistical Learning
#       by Trevor Hastie, Robert Tibshirani, and Jerome Friedman
#   https://web.stanford.edu/~hastie/Papers/ESLII.pdf（也可以免费在线阅读）。
