...
```{R frequency with geom_histogram()}
ggplot(data = diamonds) +
    geom_histogram(
        mapping = aes(x = carat),
...
```{R frequency with geom_freqpoly()}
...
```{R NA & ggplot(), results = "hide", message = FALSE,}
...
```{R geom_freqpoly() with frequency}
